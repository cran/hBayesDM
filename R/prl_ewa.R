...
#' output <- prl_ewa("example", 2000, 1000, 3, 3)
#' 
...
prl_ewa <- function(data          = "choose",
  ...
  for (i in 1:numSubjs) {
    ...
    rewlos[i, 1:useTrials] <- tmp$outcome #ifelse(tmp$outcome > 0, 1, 0)
  }
  ...
  for (i in 1:numSubjs) {
    if (indPars=="mean") {
      ...
      allIndPars[i, ] <- c( as.numeric(modeest::mlv(phi[, i], method="shorth")[1]),
